...
mapeo_ciudades = {
    1: "AGUASCALIENTES", 2: "MEXICALI", 3: "TIJUANA", 4: "LA PAZ", 5: "SAN FRANCISCO DE CAMPECHE",
    6: "SALTILLO", 7: "LA LAGUNA", 8: "PIEDRAS NEGRAS", 9: "COLIMA", 10: "MANZANILLO",
    11: "TUXTLA GUTIERREZ", 12: "TAPACHULA", 13: "CHIHUAHUA", 14: "CIUDAD JUAREZ", 19: "DURANGO",
    20: "LEON DE LOS ALDAMA", 21: "ACAPULCO DE JUAREZ", 22: "CHILPANCINGO DE LOS BRAVO", 23: "PACHUCA DE SOTO",
    25: "PUERTO VALLARTA", 26: "TOLUCA DE LERDO", 27: "ECATEPEC DE MORELOS", 28: "CIUDAD NEZAHUALCOYOTL",
    29: "MORELIA", 30: "URUAPAN", 31: "LAZARO CARDENAS", 32: "CUERNAVACA", 33: "TEPIC",
    35: "OAXACA DE JUAREZ", 36: "HEROICA PUEBLA DE ZARAGOZA", 37: "QUERETARO", 38: "CANCUN", 39: "SAN LUIS POTOSI",
    40: "CULIACAN ROSALES", 41: "MAZATLAN", 42: "LOS MOCHIS", 43: "HERMOSILLO", 44: "NOGALES",
    45: "VILLAHERMOSA", 46: "TAMPICO", 47: "REYNOSA", 48: "NUEVO LAREDO", 49: "TLAXCALA DE XICOHTENCATL",
    50: "VERACRUZ", 51: "COATZACOALCOS", 52: "MERIDA", 53: "ZACATECAS", 54: "FRESNILLO",
    55: "LOS CABOS", 56: "CIUDAD DEL CARMEN", 57: "GUANAJUATO", 58: "IXTAPA-ZIHUATANEJO", 59: "GUADALAJARA",
    60: "TONALA", 61: "TLAJOMULCO DE ZUNIGA", 62: "SAN PEDRO TLAQUEPAQUE", 63: "ZAPOPAN", 64: "MONTERREY",
    65: "SAN PEDRO GARZA GARCIA", 66: "APODACA", 67: "GUADALUPE", 68: "GENERAL ESCOBEDO", 69: "SAN NICOLAS DE LOS GARZA",
    70: "SANTA CATARINA", 71: "AZCAPOTZALCO", 72: "COYOACAN", 73: "CUAJIMALPA DE MORELOS", 74: "GUSTAVO A. MADERO",
    75: "IZTACALCO", 76: "IZTAPALAPA", 77: "LA MAGDALENA CONTRERAS", 78: "MILPA ALTA", 79: "ALVARO OBREGON",
    80: "TLAHUAC", 81: "TLALPAN", 82: "XOCHIMILCO", 83: "BENITO JUAREZ", 84: "CUAUHTEMOC",
    85: "MIGUEL HIDALGO", 86: "VENUSTIANO CARRANZA", 87: "TLALNEPANTLA DE BAZ", 88: "NAUCALPAN DE JUAREZ",
    89: "CUAUTITLAN IZCALLI", 90: "ATIZAPAN DE ZARAGOZA", 91: "CHIMALHUACAN", 92: "IRAPUATO", 93: "CHETUMAL",
    94: "CIUDAD OBREGON", 95: "CIUDAD VICTORIA", 96: "XALAPA"
}
...
def calcular_porcentaje(df, col, tipo):
    if tipo in ["Conocimiento de programas de prevención contra la violencia/delincuencia", "Incivilidades en los alrededores de su vivienda"]:
        ...
        resumen["PORCENTAJE"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * resumen["TOTAL_SI"] / resumen["TOTAL_VALIDOS"]).round(2), 0.0)
        ...
        if "PESO_SI" in df_val.columns:
            resumen["PORCENTAJE"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * resumen["TOTAL_SI"] / resumen["TOTAL_VALIDOS"]).round(2), 0.0)
        else:
            resumen["PORCENTAJE"] = 0.0

        if tipo == "Expectativas sobre delincuencia":
            resumen["PORCENTAJE_IGUAL"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * resumen["TOTAL_IGUAL"] / resumen["TOTAL_VALIDOS"]).round(2), 0.0)
            resumen["PORCENTAJE_EMPEORARA"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * resumen["TOTAL_EMPEORARA"] / resumen["TOTAL_VALIDOS"]).round(2), 0.0)
            resumen["PORCENTAJE_TOTAL"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * (resumen["TOTAL_IGUAL"] + resumen["TOTAL_EMPEORARA"]) / resumen["TOTAL_VALIDOS"]).round(2), 0.0)

    resumen["PERIODO"] = resumen["ANIO"].astype(str) + "-" + resumen["TRIMESTRE"].astype(str)
    return resumen.sort_values(["ANIO", "TRIMESTRE"])


def calcular_ranking_todas_ciudades(df, col, tipo):
    """Calcula el ranking de todas las ciudades para una variable y periodo dados."""
    if tipo in ["Conocimiento de programas de prevención contra la violencia/delincuencia", "Incivilidades en los alrededores de su vivienda"]:
        df_val = df[df[col].isin([1, 2, 9])].copy()
        df_val["PESO_SI"] = (df_val[col] == 1) * df_val["FAC_SEL"]
        resumen = df_val.groupby(["NOMBRE_CIUDAD", "ANIO", "TRIMESTRE"]).agg(
            TOTAL_VALIDOS=('FAC_SEL', 'sum'), TOTAL_SI=('PESO_SI', 'sum')
        ).reset_index()
        resumen["PORCENTAJE"] = np.where(resumen["TOTAL_VALIDOS"] > 0, (100 * resumen["TOTAL_SI"] / resumen["TOTAL_VALIDOS"]).round(2), 0.0)
        ...
        if tipo == "Percepción de inseguridad":
            ...
            # Para ranking, sumamos IGUAL + EMPEORARA como el valor principal a rankear
            df_val["PESO_IGUAL"] = (df_val[col] == 3) * df_val["FAC_SEL"]
            df_val["PESO_EMPEORARA"] = (df_val[col] == 4) * df_val["FAC_SEL"]
            df_val["PESO_SI"] = df_val["PESO_IGUAL"] + df_val["PESO_EMPEORARA"]
        else:
            df_val["PESO_SI"] = 0
    ...
    # Calcular ranking por periodo (descendente: el valor más alto es el rango 1)
    resumen = resumen.sort_values(["PERIODO", "PORCENTAJE"], ascending=[True, False])
    resumen["RANKING"] = resumen.groupby("PERIODO").cumcount() + 1
    
    return resumen[["PERIODO", "NOMBRE_CIUDAD", "PORCENTAJE", "RANKING"]]


# ===================== INTERFAZ Y VISUALIZACIÓN =====================

st.title("📊 Histórico ENSU - Inseguridad, Hábitos, Expectativas y Efectividad")
st.markdown("""
Explora el **histórico trimestral (2016–2025)** sobre percepción de inseguridad, hábitos, efectividad de autoridades y más.
""")
...
nombres_ciudades = ["TOTAL NACIONAL"] + sorted(df[df["NOMBRE_CIUDAD"] != "Desconocido"]["NOMBRE_CIUDAD"].unique())

col1, col2 = st.columns(2)
with col1:
    ciudad_1 = st.selectbox("Ciudad 1", nombres_ciudades, index=0)
with col2:
    ciudad_2 = st.selectbox("Ciudad 2", nombres_ciudades, index=1 if len(nombres_ciudades) > 1 else 0)
...
if resumen1.empty or resumen2.empty:
    st.warning(f"No hay datos válidos para '{variable_sel}' en alguna de las ciudades seleccionadas.")
else:
    st.subheader(f"Comparación de {variable_sel}: {ciudad_1} vs {ciudad_2}")
    
    # 🏆 Calcular ranking de todas las ciudades para este indicador
    ranking_df = calcular_ranking_todas_ciudades(df, variable_col, tipo_variable)
    
    rank_1 = ranking_df[ranking_df["NOMBRE_CIUDAD"] == ciudad_1][["PERIODO", "RANKING"]].rename(columns={"RANKING": f"Ranking {ciudad_1}"})
    rank_2 = ranking_df[ranking_df["NOMBRE_CIUDAD"] == ciudad_2][["PERIODO", "RANKING"]].rename(columns={"RANKING": f"Ranking {ciudad_2}"})

    # Caso especial: Expectativas sobre delincuencia
    if tipo_variable == "Expectativas sobre delincuencia":
        comparacion = resumen1[["PERIODO", "PORCENTAJE_TOTAL"]].rename(columns={"PORCENTAJE_TOTAL": ciudad_1})
        comparacion = comparacion.merge(resumen2[["PERIODO", "PORCENTAJE_TOTAL"]].rename(columns={"PORCENTAJE_TOTAL": ciudad_2}), on="PERIODO", how="outer")
        comparacion = comparacion.merge(resumen_nacional[["PERIODO", "PORCENTAJE_TOTAL"]].rename(columns={"PORCENTAJE_TOTAL": "TOTAL NACIONAL"}), on="PERIODO", how="outer")
        
        # Agregar rankings
        comparacion = comparacion.merge(rank_1, on="PERIODO", how="left")
        comparacion = comparacion.merge(rank_2, on="PERIODO", how="left")
        
        # Ordenar columnas para mejor lectura
        cols = ["PERIODO", ciudad_1, ciudad_2, "TOTAL NACIONAL", f"Ranking {ciudad_1}", f"Ranking {ciudad_2}"]
        comparacion = comparacion[[c for c in cols if c in comparacion.columns]]
        
        st.dataframe(comparacion, use_container_width=True)
        
        # Graficar solo los porcentajes (no los rankings)
        grafica = comparacion.set_index("PERIODO")
        cols_a_graficar = [c for c in [ciudad_1, ciudad_2, "TOTAL NACIONAL"] if c in grafica.columns]
        st.line_chart(grafica[cols_a_graficar])

    # Todas las demás variables
    else:
        comparacion = resumen1[["PERIODO", "PORCENTAJE"]].rename(columns={"PORCENTAJE": ciudad_1})
        comparacion = comparacion.merge(resumen2[["PERIODO", "PORCENTAJE"]].rename(columns={"PORCENTAJE": ciudad_2}), on="PERIODO", how="outer")
        comparacion = comparacion.merge(resumen_nacional[["PERIODO", "PORCENTAJE"]].rename(columns={"PORCENTAJE": "TOTAL NACIONAL"}), on="PERIODO", how="outer")
